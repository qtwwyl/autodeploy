...
const set = {
    ...
    defaultEvn: 3, //默认为测试环境
    defaultBranch: 'dev', //默认dev分支
    executablePath: path.join(__dirname, '../../chrome-win32/chrome.exe'),
    isHeadless: false,
}
const pwdSet = require('../../script/setting.js');
...
let fb = async function () {
    ...
    try {
        ...
        console.log(`工程：${process.argv[2]} 环境：${environment} 分支：${curBranch}`);
        console.log("开始构建");
        ...
        try {
            ...
            let title = '[title=' + process.argv[2] + ']'
        ...
        const secondtitle = '[title=' + process.argv[3] + environment + ']'
        let secondFilter = ''
        await new Promise((resolve) => {
